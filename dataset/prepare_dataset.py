...
def extractor_savor(dir_path, list_of_data_path, client_idx, split='train'):
    for data_idx, each_data_path in enumerate(list_of_data_path):
        ...
        save_dir_data = os.path.join(dir_path, f"dataset_{split}", f'client{client_idx}', 'data_npy' )
        save_dir_amp = os.path.join(dir_path, f"dataset_{split}", f'client{client_idx}', 'freq_amp_npy' )
